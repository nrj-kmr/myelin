    ...
    <div className='flex flex-col gap-3 h-full overflow-hidden animate-fadeIn'>
      <div className='flex justify-between items-center'>
        ...
        {googleConnected && (
          ...
          >
            <RefreshCw
              className={`w-3 h-3 ${loadingEmails ? 'animate-spin text-secondary' : ''}`}
            />
          </button>
        )}
      </div>
              ...
              <div className='flex flex-col flex-1 gap-2 pr-2 pb-4 overflow-y-auto'>
                {emails.map((mail, idx) => (
                  <div
                    ...
                    className='group/item flex flex-col gap-1 bg-muted/40 hover:bg-muted/75 p-2 border border-border/40 rounded-md w-full overflow-hidden transition-all cursor-pointer'
                  >
